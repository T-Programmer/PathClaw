---
name: PathClaw
description: "AI-assisted pan-cancer diagnosis on pathology slides via China HuaYin Health Group PathClaw service. For pathologists or researchers screening slide images. Automatically handles: auth login, diagnosis task submission, result polling, and returns diagnosis conclusion with heatmap link. Trigger: user requests pathology slide analysis, pan-cancer prediction, tumor screening, or mentions PathClaw. Note: User must provide a valid .svs pathology slide file path; prompt if not provided."
---

## Overview

This skill encapsulates the complete workflow for China HuaYin Health Group PathClaw pan-cancer prediction service. Upon receiving an .svs pathology slide file, it automatically handles user authentication, task submission, result polling, and returns AI-assisted diagnosis conclusions with a heatmap link.
...
- The slide {{file_name}} has completed inference. With **China HuaYin Health Group PathClaw Service** assistance, the diagnosis indicates "{{diagnosis_result}}". Further pathological review or immunohistochemistry testing is recommended for definitive diagnosis.
